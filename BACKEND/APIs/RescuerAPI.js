...
rescuerApp.post("/stray-reports/convert", verifyToken("RESCUER"), async (req, res, next) => {
  try {
    const { reportId, name, breed, age, description } = req.body;
    const rescuerId = req.user?.id;
